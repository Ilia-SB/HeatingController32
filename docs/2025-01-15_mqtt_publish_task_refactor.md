...
## Overview
Refactored MQTT publishing to run in a dedicated FreeRTOS task (`taskMqttPublish`) that processes publish requests from a queue-based system. This improves system responsiveness and prevents blocking operations during MQTT publishing.

## Changes Made

### 1. Queue Structure and Global Variables
Added in `src/main.cpp`:
- `MqttPublishMessage` struct with fields: topic (128 chars), payload (1024 chars), retain (bool)
- Global `QueueHandle_t mqttPublishQueue` for message queue
- Global `TaskHandle_t hndlMqttPublish` for task handle
- `MQTT_PUBLISH_QUEUE_SIZE` constant set to 20 messages
- `taskMqttPublishStackWatermark` for stack monitoring

### 2. Helper Function
Added `queueMqttPublish()` function:
- Takes topic, payload, and retain flag as parameters
- Creates `MqttPublishMessage` struct and adds to queue
- Non-blocking queue send with error logging if queue is full
- Returns success/failure boolean

### 3. New Task Implementation
Created `taskMqttPublish()` task:
...
Updated these locations to use `queueMqttPublish()`:
...
Added in `setup()`:
- Queue creation: `xQueueCreate(MQTT_PUBLISH_QUEUE_SIZE, sizeof(MqttPublishMessage))`
- Task creation: `xTaskCreate(taskMqttPublish, "MqttPublish", 4096, NULL, 1, &hndlMqttPublish)`
